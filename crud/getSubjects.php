...
$result = connection()->query("SELECT * FROM maestra_materias GROUP BY codigo UNION SELECT * FROM maestra_materias_mt GROUP BY codigo");
...
while($row = mysqli_fetch_assoc($result)){
    ...
    $emparray['data'][] = array(
        ...
        'codigoMateria'=> $row['codigo'], 
        'materia'=> $row['materia'],
        'acciones'=> '<form action="crud/insertSubjects.php" method="POST">
        <input name="materia" type="hidden" value="'.$row['codigo'].'">
        <button type="submit" class="btn btn-sm btn btn-success"><i class="bi bi-bookmark-check"></i></button>
        </form>',         
